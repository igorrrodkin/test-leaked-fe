...
import WorkspaceVerificationRow from '@/components/AddOrder/Workspace/WorkspaceVerificationRow';
...
const WorkspaceVerificationTable: FC<Props> = ({ currentRegion }) => {
    ...
    if (allPossibleChoose.length && verifyResponses?.length) {
      const updated = orderProducts.map((el, index) => ({
        ...
        isChosen:
          ...
          || (verifyResponses?.[index]?.data.message === "The verification service is unavailable and can't process your request"
          && !verifyResponses?.[index]?.data?.titles?.length)
            ? false
            : target.checked,
      }));
  ...
  const chunked = useMemo(() => {
    const arr1 = orderProducts
      .filter((orderProduct) => orderProduct.isUnable && !orderProduct.isOnVerify)
      .map(
        (orderProduct) => async function request() {
...
export default WorkspaceVerificationTable;
