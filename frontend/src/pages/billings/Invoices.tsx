...
import InvoicesTable from '@/pages/billings/InvoicesTable';
import { IInvoiceToPay } from '@/pages/billings/PaymentCredits';
...
const Invoices: FC = () => {
  ...
  const payNow = async (orgId: number, invoiceId: number, invoiceNumber: string) => {
    try {
      ...
      await dispatch(payNowAction(orgId, invoiceNumber, true));
  ...
  const invoicesWithAppliedFilters = useMemo(
    () => invoices
      ?.filter((item) => {
        ...
        return item.organisationId === orgItem.id;
      }).filter((item) => {
    ...
    [search, invoices, startDate, endDate, status, orgItem],
  );
  ...
  const isFiltered = (startDate && endDate) || status || orgItem;
