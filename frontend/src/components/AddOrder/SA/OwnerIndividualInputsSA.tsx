...
const OwnerIndividualInputsSA = () => {
  ...
  useEffect(() => {
    if (data?.region === ExistingRegions.SA) {
  ...
  const search = async () => {
    try {
      ...
      const orderItemInputs: any = {
        ...(firstName ? { givenNames: firstName.trim() } : {}),
        orgOrSurname: lastName.trim(),
        matterReference: matter,
      };

      dispatch(orderActions.setVerifyTempData({
        identifier: 'HTONSS1',
        description: `${lastName} ${firstName}`.trim(),
      }));
...
export default OwnerIndividualInputsSA;
