...
import { IMatterAws, OrderStatusEnum } from '@/store/reducers/user';
...
const FiltersMattersTable: React.FC<Props> = ({
  matters,
  users,
  mattersTotal,
  organisations,
  ...
  const maxPages = useMemo(
    () => Math.ceil(mattersTotal / limits[limit]),
    [limit, mattersTotal],
  );
  ...
  return (
    <FiltersMattersTableStyled>
      ...
      />
      <LoadingContainer isLoading={isMattersLoading}>
        {matters.length ? (
          <MattersTable
            matters={matters}
          />
...
const FiltersMattersTableStyled = styled.div`
  display: flex;
  flex: 1;
...
export default FiltersMattersTable;
