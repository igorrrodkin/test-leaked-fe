...
const VolumeFolioInputs: React.FC = () => {
  ...
  const isLotError = useMemo(() => (
    servicesValidation(lot.trim(), productId, 'Lot', true)
  ), [lot, isFirstRender]);

  const isTownError = useMemo(() => (
    servicesValidation(town.trim(), productId, 'Town', true)
  ), [town, isFirstRender]);
    ...
    <>
      <VolumeFolio ref={inputsRef}>
        ...
        <Input
          ...
          onChange={setTown}
          inputMarginBottom="0"
          isError={isButtonPressed && !!isTownError}
          errorMessage={isTownError}
          isRequired
          required
        />
        <ServiceButton
